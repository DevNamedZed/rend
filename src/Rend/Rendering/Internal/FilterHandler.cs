...
{
    /// <summary>
    /// Handles CSS filter property. Supports opacity() via render target opacity.
    /// Other filter functions (blur, brightness, etc.) are parsed but not rendered
    /// in PDF output (graceful degradation).
    /// </summary>
    internal static class FilterHandler
    {
        public static bool Apply(LayoutBox box, IRenderTarget target)
        {
            ...
            // Extract opacity filter value if present
